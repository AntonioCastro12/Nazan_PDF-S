...
export const addIdToData = (data: any) => {
  return data.map((obj: any, index: number) => ({ ...obj, [ID_DATA_NAME]: index + 1 }));
...
const convertFunctions: ConvertFunctions = {
  ...
  date: ((val: string, format: string) => DateTime.fromISO(val).toFormat(format)) as ConvertFunction<string, string>,
}

export function formatArrayValues(arr: any[], conversionConfig: ConversionConfig): any[] {
  return arr.map(obj => {

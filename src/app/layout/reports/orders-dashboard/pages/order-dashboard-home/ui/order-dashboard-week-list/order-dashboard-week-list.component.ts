...
@Component({
  selector: 'order-dashboard-week-list',
  standalone: false,
  template: `
    <section class="w-full">
      <p-card header="{{ TEMPLATE_TEXT.title }}">
        <p-table
          #dt2
          [value]="
            _ordersDashboard.state.orderStateInfo.week
        ...
        >
          <ng-template pTemplate="header">
            <tr class="text-sm">
              <th class="text-center bg-green-500">
                {{ TEMPLATE_TEXT.store }}
              </th>
              <th class="text-center bg-green-500">
                {{ TEMPLATE_TEXT.order }}
...
export class OrderDashboardWeekListComponent {
